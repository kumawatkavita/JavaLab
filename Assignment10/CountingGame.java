...
public class CountingGame
{
    ...
    public static void main(String args[])
    {
       ...
       for(int i=1;i<=max;i++) {
        ...
        //CALL DO RHYME FUNCTION UNTIL WE FIND THE WINNER:
   ...
   public static int doRhyme(ListInterface<Integer> players, ListInterface<String> rhyme, int startAt)
    {
	   ...
	   int k=startAt;
        ...
        for(int i=1;i<=rhyme.getLength();i++) {
        	...
        	//WHEN INDEX REACHES LAST PLAYER START WITH PLAYER ONE
        	if(k>players.getLength()) {
        		k=1;
        	}
        	//DISPLAY PLAYER AND ITS ENTRY:
        	System.out.println("Player " + players.getEntry(k) + ":" + rhyme.getEntry(i));
        	k++;
        }
        // RETURN POSITION OF PLAYER TO BE REMOVED:
        k--;
        System.out.println("Removing player " +  players.getEntry(k));
        players.remove(k);

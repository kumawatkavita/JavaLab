...
public class StackSort {
    ...
    private static int[] doStackSort(int data[]) {
    	...
    	//CREATING TWO STACKS :LOWER-VALUES AND UPPER-VALUES.
        ...
        for(int i:data) {
        	...
        	//IF ARRAY ELEMENT IS GREATER THAN TOP OF UPPER-VALUE STACK POP FROM UPPERSTACK AND PUSH IT TO LOWER VALUE STACK 
            if(!upperValues.isEmpty() && i>upperValues.peek()) {
        		while(!upperValues.isEmpty() && i>upperValues.peek()) {
        			lowerValues.push(upperValues.pop());
        		}
        	}
            
          //IF ARRAY ELEMENT IS LESS THAN TOP OF LOWER-VALUE STACK POP FROM LOWERSTACK AND PUSH IT TO UPPER VALUE STACK
        	else if(!lowerValues.isEmpty() && i<lowerValues.peek()) {
        		while(!lowerValues.isEmpty() && i<lowerValues.peek()) {
        			upperValues.push(lowerValues.pop());
        	}
        	}
            
            //PUSH THE ELEMENT TO UPPERVALUE STACK
            upperValues.push(i);   	
        }
        
        //PUSH THE REMAINING ELEMENTS OF LOWERVALUE STACK TO UPPERVALUE STACK
        while(!lowerValues.isEmpty()) {
        	upperValues.push(lowerValues.pop());
        }
        
        
        //POP FROM UPPERVALUE STACK AND STORE IN THE RESULT ARRAY
    	int j=0;
